...
page_remove_pte(pde_t *pgdir, uintptr_t la, pte_t *ptep) {
     ...
     *
     *也许你需要帮助评论，下面的评论可以帮助你完成代码
     *
     * 一些有用的宏和定义，您可以在下面的实现中使用它们。
     * 宏或函数：
     * 结构页 *页 pte2page（*ptep）：从 ptep 的值中获取相应的页面
     * free_page：释放页面
     * page_ref_dec（页面）：减少页面>参考注意：ff 页面>ref == 0，那么这个页面应该是免费的。
     * tlb_invalidate（pde_t *pgdir， uintptr_t la） ： 使 TLB 条目无效，但仅当页表是
     * 编辑的是处理器当前正在使用的那些。
     *定义：
